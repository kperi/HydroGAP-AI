...
def save_combined_plot_full(results_folder, val_full, real_predictions, input_file_name, model_type, plot_start_date, plot_end_date, rotation=45, tick_interval='auto'):
    ...
    plt.plot(val_full.index.values, val_full['predicted'].values, color='red', label='Predicted (synth gaps only)')
    # Plot actual values with a black line
    plt.plot(val_full.index.values, val_full['obsdis'].values, color='black', label='Actual Values (with original gaps)')
    # Plot the aligned real predictions with a green line, showing predictions only for the gaps
    plt.plot(aligned_predictions.index.values, aligned_predictions.values, color='green', label='Predicted (real gaps only)')
    ...
    plt.title(f'Actual vs Predicted Values - ({model_type.upper()})\n{input_file_name}', fontsize=20)
    ...
    plot_file_name = os.path.join(results_folder, f"real_vs_pred_full_{model_type}_{os.path.splitext(os.path.basename(input_file_name))[0]}.png")
    plt.savefig(plot_file_name, dpi=300)
    plt.close()


def save_combined_plot_per_year(results_folder, val_full, real_predictions, input_file_name, model_type, rotation=45, tick_interval='auto'):
    # Create a subfolder for the current file inside the results folder
    file_folder_name = os.path.splitext(input_file_name)[0]  # Get the base filename (without extension)
    file_specific_folder = os.path.join(results_folder, file_folder_name)  # Create the path for the file-specific folder
    os.makedirs(file_specific_folder, exist_ok=True)  # Create the folder if it doesn't exist
    ...
    for year, group in grouped_by_year:
        ...
        common_index = group.index.intersection(real_predictions.index)  # Find common dates between group and real_predictions
        aligned_predictions.loc[common_index] = real_predictions.loc[common_index]  # Only assign where the indices match
        
        # Plot predicted values with a red line
        plt.plot(group.index.values, group['predicted'].values, color='red', label='Predicted Values (synth gaps only)')
        # Plot actual values with a black line
        plt.plot(group.index.values, group['obsdis'].values, color='black', label='Actual Values (with original gaps)')
        ...
        plt.ylabel('Discharge', fontsize=15)
        plt.legend(fontsize=12)
        plt.title(f'Actual vs Predicted Values - {year} ({model_type.upper()})\n{input_file_name}', fontsize=20)
        ...
        plot_file_name = os.path.join(file_specific_folder, f"real_vs_pred_{year}_{model_type}.png")
        plt.savefig(plot_file_name, dpi=300)
        plt.close()
